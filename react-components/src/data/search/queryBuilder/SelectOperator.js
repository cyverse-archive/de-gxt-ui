...
function SelectOperator(props) {
    const { operators, classes, parentId } = props;

    return (
        <Grid item>
            <Fields
                names={["opLabel", "exact", "negated", "permission_recurse"]}
                classes={classes}
                parentId={parentId}
                operators={operators}
...
function handleChange(event, opLabel, exact, negated, permission_recurse) {
    ...
    if (permission_recurse)
        permission_recurse.input.onChange(
            operatorMap[operator].permission_recurse
        );
}
