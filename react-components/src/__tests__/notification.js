...
import DenyJoinRequestDetailsDialogTest
    from "../../stories/notifications/view/dialogs/DenyJoinRequestDetailsDialog.stories";
import JoinTeamRequestDialogTest
    from "../../stories/notifications/view/dialogs/JoinTeamRequestDialog.stories";
...
it('renders DenyJoinRequestDetailsDialogTest without crashing', () => {
    const div = document.createElement('div');
    ReactDOM.render(<MuiThemeProvider
        theme={getDefaultTheme()}><DenyJoinRequestDetailsDialogTest/></MuiThemeProvider>, div);
    ReactDOM.unmountComponentAtNode(div);
});
it('renders JoinTeamRequestDialogTest without crashing', () => {
    const div = document.createElement('div');
    ReactDOM.render(<MuiThemeProvider
        theme={getDefaultTheme()}><JoinTeamRequestDialogTest/></MuiThemeProvider>, div);
    ReactDOM.unmountComponentAtNode(div);
});
it('renders RequestHistoryDialogTest without crashing', () => {
    const div = document.createElement('div');
    ReactDOM.render(<MuiThemeProvider
        theme={getDefaultTheme()}><RequestHistoryDialogTest/></MuiThemeProvider>, div);
    ReactDOM.unmountComponentAtNode(div);
});
