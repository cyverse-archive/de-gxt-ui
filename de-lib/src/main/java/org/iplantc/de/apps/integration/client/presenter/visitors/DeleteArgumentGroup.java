...
public class DeleteArgumentGroup extends EditorVisitor {
    ...
    public DeleteArgumentGroup(ArgumentGroup argGrp, AppsEditorView.AppsEditorViewAppearance appearance) {
        this.argGrp = argGrp;
    ...
    public <T> void endVisit(EditorContext<T> ctx) {
        if(ctx.getEditor() instanceof AppTemplateForm){
            ...
            if (argumentGroups.getList().isEmpty()) {
                ...
                toBeSelected.asWidget().addStyleName(appearance.appHeaderSelect());
            }
        }
    }
}
