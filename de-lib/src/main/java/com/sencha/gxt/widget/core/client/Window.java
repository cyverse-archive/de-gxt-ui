...
/**
 * <p>A specialized content panel intended for use as an application window.</p>
 *
 * <p>Code snippet:</p>
 *
 * <pre><code>
 * Over_Window w = new Over_Window();
 ...
 * </code></pre>
 *
 * <p>In this case, the window contains a single Frame widget, as specified in the call to <code>setWidget</code>. To
 ...
 * instruct the window to update its layout.</p>
 *
 * <p>The {@link #setPixelSize(int, int)} method specifies the size of the window. This size, adjusted for the frame and
 ...
 * fashion, as specified by the child's {@link LayoutData}.</p>
 *
 * <p>Alternatively, either dimension in the call to <code>setPixelSize</code> may be specified as -1. This enables auto
 ...
 * of the window, in a bottom-up fashion.</p>
 *
 * <p>To configure auto size for a particular dimension, you must also set the minimum size for that dimension to zero and
 * turn off the resizable property so that the user cannot resize the window (which would turn auto size off). For
 * example, to auto size both the width and the height, set the following window properties as indicated:</p>
 *
 * <pre><code>
 * w.setPixelSize(-1, -1);
 * w.setMinWidth(0);
 ...
 * </code></pre>
 *
 * <p>Note: even with auto size configured, you must still invoke {@link #forceLayout()} as described above if you add or
 ...
 * ensure that the size of the window cannot grow beyond the viewport (e.g. the browser window).</p>
 */


/**
 * CyVerse team: Why is this here?  It's a workaround for GXT 4.0.2 to prevent the Tab key toggling windows instead of selecting the
 * next field/widget
 * https://www.sencha.com/forum/showthread.php?309000-Tab-key-doesn-t-go-to-the-next-field
 */
public class Window extends ContentPanel implements HasActivateHandlers<Window>, HasDeactivateHandlers<Window>,
    ...
    public Window(WindowAppearance appearance) {
        ...
        eventPreview = new BaseEventPreview() {
            ...
            protected void onPreviewKeyPress(NativePreviewEvent event) {
                ...
                // ~~~ workaround - fix tabbing per window
                Widget a = manager.getActive();
                if (a instanceof Window) {
                    Window active = (Window) a;

                    if (active != null && active == Window.this) {
    ...
    protected Layer ghost() {
        XElement bodyWrap = getAppearance().getBodyWrap(getElement());
        int w = bodyWrap.getOffsetWidth();
        int h = bodyWrap.getOffsetHeight();


        Layer g = createGhost();
        XElement el = g.getElement();
        el.setVisibility(false);
        el.setWidth(w, true);
        el.getChild(1).<XElement>cast().setHeight(h - 1, true);
        return g;
    }
    ...
    protected void onWindowResize(int width, int height) {
        if (!wasLastWindowResizeForTouchKeyboardAdjustPan()) {
            if (isVisible()) {
                if (maximized) {
                    fitContainer();
                } else {
    ...
    /**
     * Delegate to {@link Viewport#wasLastWindowResizeForTouchKeyboardAdjustPan()} if a {@link Viewport} is in use,
     * otherwise return false.
     *
     * @return delegate to {@link Viewport#wasLastWindowResizeForTouchKeyboardAdjustPan()} and return the result
     * @see Viewport#wasLastWindowResizeForTouchKeyboardAdjustPan()
     */
    private boolean wasLastWindowResizeForTouchKeyboardAdjustPan() {
        Widget vp = RootPanel.get().getWidgetCount() > 0 ? RootPanel.get().getWidget(0) : null;
        if (vp instanceof Viewport) {
            return callLastWindowResizeForTouchKeyboardAdjustPan(vp);
        } else {
            return false;
        }
    }
    ...
    /**
     * We don't yet have a good public API for detecting screen orientation or touch keyboard changes on the viewport,
     * so we are forced to use JSNI in order to delegate to the private
     * {@link Viewport#wasLastWindowResizeForTouchKeyboardAdjustPan()} method that returns what we need.
     *
     * @param vp the viewport widget
     * @return delegate to {@link Viewport#wasLastWindowResizeForTouchKeyboardAdjustPan()} and return the result
     * @see Viewport#wasLastWindowResizeForTouchKeyboardAdjustPan()
     */
    private native boolean callLastWindowResizeForTouchKeyboardAdjustPan(Widget vp) /*-{
        return vp.@com.sencha.gxt.widget.core.client.container.Viewport::wasLastWindowResizeForTouchKeyboardAdjustPan();
    }-*/;
